...
void changeState() {
    switch (currentState) {
        case RED:
            setLights(HIGH, LOW, LOW); // Vermelho aceso
            if (millis() - stateStartTime >= RED_DURATION) {
                currentState = GREEN;
                stateStartTime = millis();
                client.publish(mqtt_topic_sync, "GREEN"); // Publica o estado para o Semáforo 2
            }
            break;

        case GREEN:
            setLights(LOW, LOW, HIGH); // Verde aceso
            if (millis() - stateStartTime >= GREEN_DURATION) {
                currentState = YELLOW;
                stateStartTime = millis();
                client.publish(mqtt_topic_sync, "YELLOW"); // Publica o estado para o Semáforo 2
            }
            break;

        case YELLOW:
            setLights(LOW, HIGH, LOW); // Amarelo aceso
            if (millis() - stateStartTime >= YELLOW_DURATION) {
                currentState = RED;
                stateStartTime = millis();
                client.publish(mqtt_topic_sync, "RED"); // Publica o estado para o Semáforo 2
            }
            break;
    }
}
